...
export const NotificationCenter: React.FC = () => {
  const { notifications, markNotificationAsRead, clearNotifications, currentUser } = usePlanet();
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const userNotifications = notifications.filter(n => n.userId === currentUser);
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button 
        ...
        className="relative p-2 rounded-xl glass hover:bg-[var(--glass-hover)] transition-all group [&>svg]:w-5 [&>svg]:h-5"
      >
        {unreadCount > 0 ? (
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-4 border-b border-[var(--glass-border)] flex items-center justify-between bg-[var(--glass)]">
              ...
              <div className="flex gap-2">
                ...
                <motion.button 
                  ...
                  className="p-1.5 rounded-lg hover:bg-[var(--glass)] transition-all"
                >
                  <X size={14} />
                </motion.button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-2 space-y-1">
              {userNotifications.length === 0 ? (
                ...
                userNotifications.map((n) => (
                  <div 
                    key={n.id}
                    className={`p-4 rounded-2xl transition-all border border-transparent hover:border-[var(--glass-border)] relative group ${n.read ? 'opacity-50' : 'bg-[var(--glass)] border-[var(--glass-border)] shadow-sm'}`}
                  >
            ...
            {userNotifications.length > 5 && (
              <div className="p-3 bg-[var(--glass)] border-t border-[var(--glass-border)] text-center">
                <motion.button 
                  whileTap={{ scale: 0.98 }}
