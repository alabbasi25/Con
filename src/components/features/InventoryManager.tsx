      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 px-2">
        <div className="space-y-3">
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 text-blue-500 text-[10px] font-black uppercase tracking-[0.2em]"
          >
            سلسلة التوريد نشطة <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-ping" />
          </motion.div>
          ...
          <p className="text-sm lg:text-base text-[var(--color-text-secondary)] font-bold max-w-md leading-relaxed opacity-70">
            تتبع احتياجات الكوكب الاستهلاكية وتأكد من أن المدار ممتلئ دائماً بالموارد.
          </p>
        </div>
        <div className="flex gap-3">
          {/* View Mode Toggle */}
          <div className="flex p-1.5 bg-[var(--glass)] rounded-2xl border border-[var(--glass-border)] shadow-inner backdrop-blur-xl">
            <button
              onClick={() => setViewMode('kanban')}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all duration-300 flex items-center gap-2 ${
                viewMode === 'kanban' ? 'bg-[var(--color-primary)] text-white shadow-lg' : 'text-[var(--text-mute)] hover:text-[var(--color-text-primary)]'
              }`}
            >
              <Package size={14} /> كانبان
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all duration-300 flex items-center gap-2 ${
                viewMode === 'list' ? 'bg-[var(--color-primary)] text-white shadow-lg' : 'text-[var(--text-mute)] hover:text-[var(--color-text-primary)]'
              }`}
            >
          ...
          <motion.button 
            ...
            onClick={() => {
              ...
              addNotification({
                title: 'تزامن الموارد',
                content: 'تم تحديث مستويات الاستهلاك بناءً على مدار الوقت.',
                type: 'routine'
              });
            }}
            className={`w-14 h-14 rounded-2xl transition-all flex items-center justify-center border-2 ${inventory.some(i => i.status === 'low') ? 'bg-rose-500 border-rose-500 text-white animate-pulse shadow-[0_0_20px_rgba(244,63,94,0.4)]' : 'bg-[var(--color-bg-card)]/40 border-emerald-500/50 text-emerald-500'} hover:scale-110 active:rotate-180 duration-500`}
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-4">
          <div className="relative">
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--text-mute)]" size={18} />
            <input 
              type="text"
              placeholder="ابحث عن سلعة..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-[var(--glass)] border border-[var(--glass-border)] rounded-2xl px-12 py-4 text-sm focus:ring-1 focus:ring-[var(--color-primary)] outline-none"
            />
          </div>
          
          <button 
            onClick={() => setShowFilters(!showFilters)}
            className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 border border-[var(--glass-border)] transition-all font-bold text-xs uppercase tracking-widest ${showFilters ? 'bg-[var(--color-primary)] text-white shadow-xl' : 'glass hover:bg-[var(--glass-hover)] opacity-70'}`}
          >
            <ListFilter size={18} />
        ...
        <AnimatePresence>
          {showFilters && (
            ...
            >
              <div className="glass-card p-6 grid grid-cols-1 md:grid-cols-3 gap-6 border-[var(--glass-border)]">
                <div className="space-y-3">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {col.id !== 'needed' && (
                        <button 
                          ...
                          className="p-1.5 rounded-md hover:bg-[var(--glass-hover)]"
                        >
