          ...
          >
            <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest opacity-40">
              ...
              <div className="flex gap-1">
                {QUESTIONS.map((_, i) => (
                  <div key={i} className={`w-4 h-1 rounded-full transition-all ${i === currentQuestionIdx ? 'bg-rose-500 w-8' : (i < currentQuestionIdx ? 'bg-rose-500/40' : 'bg-[var(--glass-hover)]')}`} />
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <button
                  ...
                  className="p-6 rounded-2xl bg-[var(--glass)] border border-[var(--glass-border)] text-sm font-black hover:bg-[var(--glass-hover)] hover:border-[var(--glass-border-hover)] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-between group"
                >
                  {btn.label}
          ...
          >
            <div className="glass-card p-12 text-center space-y-6 relative overflow-hidden">
               ...
               <div className="grid grid-cols-2 md:grid-cols-5 gap-4 pt-6">
                 {Object.entries(myResult?.scores || {}).map(([lang, score]) => (
                   <div key={lang} className="glass-card p-4 space-y-2 border-[var(--glass-border)] bg-[var(--glass)]">
            ...
            <div className="flex gap-4">
              <button 
               ...
               className="flex-1 py-4 rounded-2xl bg-[var(--glass)] border border-[var(--glass-border)] text-xs font-black hover:bg-[var(--glass-hover)] transition-all flex items-center justify-center gap-2"
              >
