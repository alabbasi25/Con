    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 px-2">
        <div className="space-y-3">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-500 text-[10px] font-black uppercase tracking-[0.2em]"
          >
            محرك المهام نشط <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
          </motion.div>
          ...
          <p className="text-sm lg:text-base text-[var(--color-text-secondary)] font-bold max-w-md leading-relaxed opacity-70">
            نسق حياتك اليومية مع شريكك وارفع نقاط "بركة" الكوكب بإنجاز المهام.
          </p>
        </div>
        <div className="flex gap-3">
          <div className="hidden md:flex flex-col items-end px-4 border-r border-[var(--glass-border)]">
            <div className="text-[10px] font-bold uppercase tracking-widest opacity-40">المستوى الحالي</div>
            <div className="text-xl font-black text-amber-500 flex items-center gap-2">
              <Trophy size={18} /> {barakahPoints} <span className="text-[10px] opacity-40 font-bold">بركة</span>
            </div>
          </div>
          <div className="flex gap-2">
            <motion.button 
              ...
              className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 border-2 ${viewMode === 'kanban' ? 'bg-[var(--color-primary)] border-[var(--color-primary)] text-white shadow-2xl shadow-[var(--color-primary)]/30' : 'bg-[var(--color-bg-card)]/40 border-[var(--glass-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:border-[var(--glass-border-hover)] hover:bg-[var(--glass)]'}`}
              title="تغيير طريقة العرض"
            >
              <ListTodo size={28} className={viewMode === 'list' ? 'opacity-100' : 'opacity-40'} />
            </motion.button>
            <motion.button 
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => setShowAnalytics(!showAnalytics)}
              className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 border-2 ${showAnalytics ? 'bg-[var(--color-primary)] border-[var(--color-primary)] text-white shadow-2xl shadow-[var(--color-primary)]/30' : 'bg-[var(--color-bg-card)]/40 border-[var(--glass-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:border-[var(--glass-border-hover)] hover:bg-[var(--glass)]'}`}
            >
              <BarChart3 size={28} />
            </motion.button>
            <motion.button 
              ...
              className="w-14 h-14 rounded-2xl glass text-amber-500 flex items-center justify-center border-amber-500/20 shadow-xl shadow-amber-500/10"
            >
              <RotateCw size={28} />
            </motion.button>
            <motion.button 
              ...
              className="w-14 h-14 rounded-2xl bg-[var(--color-primary)] text-white flex items-center justify-center shadow-2xl shadow-[var(--color-primary)]/40 hover:scale-110"
            >
              <Plus size={28} />
            </motion.button>
          </div>
      ...
      <div className="space-y-4">
        <button 
          ...
          className={`w-full glass-card p-4 border-[var(--glass-border)] flex items-center justify-between group hover:bg-[var(--glass)] transition-all ${showFilters ? 'bg-[var(--glass)]' : ''}`}
        >
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div
              ...
              className="overflow-hidden"
            >
              <div className="glass-card p-4 space-y-4 border-[var(--glass-border)]">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-bold whitespace-nowrap transition-all border ${
                          filterDaily 
                            ...
                            : 'glass text-[var(--color-text-secondary)] border-[var(--glass-border)]'
                        }`}
                      >
                        مهام اليوم والمتأخرة
                      </button>
                      <div className="w-px h-6 bg-[var(--glass-hover)] mx-1 self-center" />
                    </>
                  )}
                ...
                <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-[var(--glass-border)]">
      ...
      {viewMode === 'list' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(['needed', 'in-cart', 'purchased'] as const).map(column => {
            const columnTasks = filteredAndSortedTasks.filter(t => (t.kanbanColumn || 'needed') === column);
            return (
              <div key={column} className="glass-card p-4 space-y-4 border-[var(--glass-border)] bg-[var(--glass-active)]">
                <div className="flex items-center gap-2 px-2 pb-2 border-b border-[var(--glass-border)]">
                  <div className={`w-3 h-3 rounded-full ${column === 'needed' ? 'bg-rose-500' : column === 'in-cart' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                  <h3 className="text-sm font-black uppercase tracking-widest">
                    {column === 'needed' ? 'مطلوب (Needed)' : column === 'in-cart' ? 'في السلة (In Cart)' : 'تم الشراء (Purchased)'}
                  </h3>
                  <span className="text-[10px] font-bold opacity-40 mr-auto">{columnTasks.length}</span>
                </div>
                <div className="space-y-4">
                  {columnTasks.length === 0 && <p className="text-xs opacity-40 text-center italic py-8">لا يوجد مهام</p>}
                  {columnTasks.map(task => (
                    <EnhancedTaskCard 
                      key={task.id} 
                      task={task}
                      viewMode="kanban"
                      onMove={(newCol) => updateTaskColumn(task.id, newCol as 'needed' | 'in-cart' | 'purchased')}
                      onDelete={() => setConfirmDelete(task.id)}
                      onRoulette={() => toggleRouletteTask(task.id)}
                      isInRoulette={rouletteTasks.includes(task.id)}
                    />
...
}> = ({ task, onComplete, onDelete, onDelegate, onRoulette, isInRoulette, isPartner, viewMode = 'list', onMove }) => {
  const { priorityConfigs, getAITaskSuggestion, currentUser } = usePlanet();
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem]">
      ...
      <motion.div 
        ...
        className={`glass-card p-6 space-y-4 relative z-10 border-[var(--glass-border)] ${isPartner ? 'opacity-80' : 'cursor-grab active:cursor-grabbing'}`}
      >
        <div className="flex justify-between items-start">
          ...
          {(task.creatorId === currentUser || !isPartner) && onDelete && (
            <button 
              onClick={onDelete}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-[var(--glass-border)]">
          <div className="flex items-center gap-4">
            {task.dueDate && (
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${
                task.dueDate < Date.now() ? 'bg-rose-500/10 border-rose-500/20 text-rose-500' : 'bg-[var(--glass)] border-[var(--glass-border)] text-[var(--text-mute-hover)]'
              }`}>
                <Calendar size={12} />
          ...
          <div className="flex gap-1 items-center">
            {viewMode === 'kanban' && onMove && (
              <div className="flex bg-[var(--glass-active)] rounded-xl overflow-hidden p-0.5 mr-2">
                <button 
                  onClick={() => onMove('needed')}
                  className={`px-2 py-1 text-[8px] font-black uppercase transition-colors ${task.kanbanColumn === 'needed' || !task.kanbanColumn ? 'bg-rose-500 text-white rounded-lg' : 'text-[var(--text-mute)] hover:text-[var(--color-text-primary)]'}`}
                >مطلوب</button>
                <button 
                  onClick={() => onMove('in-cart')}
                  className={`px-2 py-1 text-[8px] font-black uppercase transition-colors ${task.kanbanColumn === 'in-cart' ? 'bg-amber-500 text-white rounded-lg' : 'text-[var(--text-mute)] hover:text-[var(--color-text-primary)]'}`}
                >بالسلة</button>
                <button 
                  onClick={() => onMove('purchased')}
                  className={`px-2 py-1 text-[8px] font-black uppercase transition-colors ${task.kanbanColumn === 'purchased' ? 'bg-emerald-500 text-white rounded-lg' : 'text-[var(--text-mute)] hover:text-[var(--color-text-primary)]'}`}
                >تم</button>
              </div>
            )}
            <button 
              ...
              <button
                ...
                className={`py-4 rounded-2xl border-2 text-[10px] font-black transition-all flex items-center justify-center gap-3 ${
                  newTask.category === c 
                    ...
                    : 'glass opacity-60 border-[var(--glass-border)] hover:border-[var(--glass-border-hover)]'
                }`}
              >
          ...
          <div className="flex gap-3">
            <button
              ...
              className={`flex-1 py-4 rounded-2xl border-2 text-[10px] font-black transition-all flex items-center justify-center gap-2 ${
                newTask.autoAssign 
                  ? 'bg-amber-500 text-white border-amber-500 shadow-lg' 
                  : 'glass opacity-60 border-[var(--glass-border)]'
              }`}
            >
      ...
      >
        ...
        <AnimatePresence>
          {winner && (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[var(--glass-hover)] to-transparent animate-shimmer" />
        ...
        <div className="glass-card p-6 space-y-4 border-[var(--glass-border)]">
          ...
          <div className="max-h-32 overflow-y-auto space-y-2 no-scrollbar">
            {tasks.map(t => (
              <div key={t.id} className="text-sm font-bold p-4 rounded-2xl bg-[var(--glass)] border border-[var(--glass-border)] text-right flex items-center justify-between group hover:bg-[var(--glass-hover)] transition-all">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={onClose} className="flex-1 glass px-6 rounded-[2rem] text-white font-black text-sm hover:bg-[var(--glass-hover)] transition-all">إغلاق</button>
        </div>
      </motion.div>
    </div>
  );
