...
export const HydrationStation: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
          ...
          <div className="grid grid-cols-3 gap-3 pt-2">
            {[200, 330, 500].map(amount => (
              <button 
                ...
                className="py-4 rounded-2xl bg-blue-500/10 text-blue-500 border border-blue-500/20 hover:bg-blue-500 hover:text-[var(--color-text-primary)] transition-all flex flex-col items-center gap-1"
              >
                <GlassWater size={20} />
      ...
      <div className={`p-6 rounded-2xl border transition-all duration-500 flex gap-4 items-center justify-between ${smartHydrationEnabled ? 'bg-amber-500/10 border-amber-500/30' : 'bg-[var(--glass)] border-[var(--glass-border)] opacity-60'}`}>
        <div className="flex gap-4 items-center">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${smartHydrationEnabled ? 'bg-amber-500/20 text-amber-500' : 'bg-[var(--glass-hover)] text-[var(--text-mute)]'}`}>
            <AlertCircle size={28} />
          </div>
          <div>
            <h3 className={`text-sm font-bold transition-colors ${smartHydrationEnabled ? 'text-amber-500' : 'text-[var(--text-mute-hover)]'}`}>تذكير ذكي</h3>
        ...
        <button 
          ...
          className={`relative w-12 h-6 rounded-full transition-colors duration-300 ${smartHydrationEnabled ? 'bg-amber-500' : 'bg-[var(--glass-hover)]'}`}
        >
          <motion.div 
