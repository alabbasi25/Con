                ...
                {Array.from({ length: project.totalSteps }).map((_, i) => (
                  <div 
                    ...
                    className={`h-1 flex-1 rounded-full transition-all duration-500 ${
                      i < project.currentStep ? 'bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.5)]' : 'bg-[var(--glass)]'
                    }`}
                  />
                ))}
              </div>
...
const PhotoCarousel: React.FC<{ photos: { url: string; timestamp: number }[] }> = ({ photos }) => {
  ...
  return (
    <div className="relative group overflow-hidden rounded-xl bg-[var(--glass-active)] aspect-video">
      ...
      {photos.length > 1 && (
        <>
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
            {photos.map((_, i) => (
              <div key={i} className={`w-1 h-1 rounded-full ${i === index ? 'bg-white' : 'bg-[var(--glass-active)]'}`} />
            ))}
          </div>
        </>
      )}
