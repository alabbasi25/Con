...
export const UnifiedLedger: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 px-1 mb-8">
        <div className="space-y-3">
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            className="glass-pill text-emerald-500 inline-flex items-center gap-2 ring-1 ring-emerald-500/20"
          >
            المنظومة المالية نشطة <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
          </motion.div>
          <h2 className="text-4xl lg:text-7xl font-black italic serif tracking-tighter leading-none">
            المالية والخزينة
          </h2>
          <p className="text-xs lg:text-base text-[var(--color-text-secondary)] font-bold max-w-sm opacity-60 leading-relaxed">
            أدر موارد كوكبك المالية بحكمة وتيقن من تدفقات الطاقة النقدية.
          </p>
        </div>
        
        {/* Tab Switcher - Improved for mobile scroll */}
        <div className="flex p-1.5 glass rounded-[2.5rem] border border-[var(--glass-border)] shadow-2xl backdrop-blur-3xl overflow-x-auto no-scrollbar max-w-full">
          <div className="flex gap-1">
            {(['summary', 'transactions', 'liabilities', 'goals'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`py-3 px-6 rounded-[2rem] text-[10px] font-black uppercase transition-all duration-500 whitespace-nowrap shrink-0 ${
                  activeTab === tab ? 'bg-white text-[var(--color-bg-deep)] shadow-2xl scale-105 z-10' : 'text-[var(--text-mute)] hover:text-[var(--color-text-primary)] hover:bg-[var(--glass)]'
                }`}
              >
        ...
        {activeTab === 'summary' && (
          ...
          >
            ...
            <div className="glass-card p-6 space-y-6 overflow-hidden relative group">
              ...
              <div className="relative h-3 w-full bg-[var(--glass)] rounded-full overflow-hidden border border-[var(--glass-border)]">
                <motion.div 
                  initial={{ width: 0 }}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button 
                ...
                className="flex-1 py-4 bg-[var(--glass)] border border-[var(--glass-border)] rounded-2xl font-black text-xs hover:bg-[var(--glass-hover)] transition-all flex items-center justify-center gap-2"
              >
        ...
        {activeTab === 'transactions' && (
          ...
          >
            <div className="flex items-center justify-between px-1">
              ...
              <button onClick={() => setShowFilters(!showFilters)} className={`p-2 rounded-xl transition-all ${showFilters ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--glass)] border border-[var(--glass-border)]'}`}><Filter size={14} /></button>
            </div>
            <AnimatePresence>
              {showFilters && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="glass-card p-4 grid grid-cols-2 gap-4 mb-4">
                    <select value={filterType} onChange={(e) => setFilterType(e.target.value as any)} className="bg-[var(--glass)] border border-[var(--glass-border)] rounded-xl px-3 py-2 text-[10px] outline-none">
                      <option value="all">كل الأنواع</option>
                      <option value="fixed">ثابت</option>
                      <option value="variable">متغير</option>
                    </select>
                    <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="bg-[var(--glass)] border border-[var(--glass-border)] rounded-xl px-3 py-2 text-[10px] outline-none">
            ...
            <div className="grid gap-2">
              {filteredTransactions.map(t => (
                <div key={t.id} className="glass-card p-4 flex items-center justify-between hover:bg-[var(--glass)] transition-colors group">
              ...
              {liabilities.map(l => (
                <div key={l.id} className="glass-card p-5 space-y-4 group">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-1.5 w-full bg-[var(--glass)] rounded-full overflow-hidden">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 py-2 rounded-xl bg-rose-500/10 text-rose-500 text-[10px] font-black hover:bg-rose-500 hover:text-[var(--color-text-primary)] transition-all shadow-lg"
                    >
                      سدد الآن
                    </button>
                    {l.userId === currentUser && (
                ...
                <div key={goal.id} className="glass-card p-5 space-y-4 group">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 py-2 rounded-xl bg-emerald-500/10 text-emerald-500 text-[10px] font-black hover:bg-emerald-500 hover:text-[var(--color-text-primary)] transition-all shadow-lg"
                    >
                      إيداع جديد
                    </button>
