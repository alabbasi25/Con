...
import { TrendingUp, Target, ShieldCheck, Lock, Unlock, Image as ImageIcon, CheckCircle2, XCircle, Coins, ArrowUpRight, Timer, Plus, X, Trash2 } from 'lucide-react';
import { usePlanet } from '../../context/KokabContext';
import { ModernInput } from '../ui/ModernInput';

export const FutureFund: React.FC = () => {
  const { assets, unlockAsset, approveVisionBoard, currentUser, coinStaking, stakeCoins, addFinancialGoal, deleteAssetGoal } = usePlanet();
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <section className="glass-card p-6 border-amber-500/20 bg-amber-500/5 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-2xl bg-[var(--glass)] border border-[var(--glass-border)] space-y-1">
        ...
        <div className="flex gap-3">
          ...
          <div className="px-4 rounded-2xl bg-[var(--glass)] border border-[var(--glass-border)] flex items-center justify-center text-amber-500">
            <Timer size={18} />
          </div>
        </div>
        ...
        {assets.map(asset => {
          ...
          return (
            <div key={asset.id} className="glass-card overflow-hidden flex flex-col">
              ...
              <div className="relative h-48 bg-[var(--glass-active)] overflow-hidden">
                {asset.imageUrl ? (
                  <motion.img 
                    src={asset.imageUrl} 
                    alt={asset.name}
                ...
                {asset.userId === currentUser && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); deleteAssetGoal(asset.id); }}
                    className="absolute top-4 right-4 p-2 rounded-xl bg-[var(--glass-active)] backdrop-blur-md text-[var(--text-mute)] hover:text-rose-500 transition-all z-10"
                  >
                    <Trash2 size={16} />
                  </button>
                )}
                ...
                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                  <div className="space-y-1">
                    ...
                    <p className="text-[var(--text-mute-hover)] text-[10px] font-medium max-w-[200px]">{asset.visualDescription}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-white font-black text-2xl">{Math.round(progress)}%</div>
                    <div className="text-[var(--text-mute)] text-[8px] font-bold uppercase tracking-widest">اكتمال الرؤية</div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-[var(--glass)] border border-[var(--glass-border)]">
                  <div className="flex items-center gap-3">
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-lg transition-all ${
                        (currentUser === 'F' ? asset.isApprovedByF : asset.isApprovedByB)
                          ...
                          : 'bg-[var(--glass-hover)] hover:bg-[var(--glass-active)]'
                      }`}
                    >
                      <CheckCircle2 size={16} />
                    </button>
                    <div className="flex items-center gap-1 ml-2">
                      <div className={`w-1.5 h-1.5 rounded-full ${asset.isApprovedByF ? 'bg-green-500' : 'bg-[var(--glass-hover)]'}`} />
                      <div className={`w-1.5 h-1.5 rounded-full ${asset.isApprovedByB ? 'bg-green-500' : 'bg-[var(--glass-hover)]'}`} />
                    </div>
                  </div>
                </div>
              ...
              <form onSubmit={handleAddGoal} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full h-24 glass border-[var(--glass-border)] rounded-xl px-4 py-3 text-xs outline-none"
                  />
                </div>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--glass)] border border-[var(--glass-border)]">
                  ...
                  <button 
                    ...
                    className={`w-12 h-6 rounded-full transition-all relative ${newGoal.requiresDualAuth ? 'bg-amber-500' : 'bg-[var(--glass-hover)]'}`}
                  >
                    <motion.div 
