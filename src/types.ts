...
export interface Task {
  ...
  kanbanColumn?: 'needed' | 'in-cart' | 'purchased';
}
...
export interface Notification {
  id: string;
  userId: UserID;
  type: 'urgent' | 'routine' | 'financial' | 'spiritual' | 'social' | 'tasks' | 'ai';
  title: string;
  content: string;
  timestamp: number;
  read: boolean;
...
export interface NotificationSettings {
  ...
  social: boolean;
  smartBudget: boolean;
  affectionNudges: boolean;
  soundEnabled: boolean;
  muteMode: 'none' | 'all' | 'silent';
}
...
export type Theme = 'midnight' | 'emerald' | 'ocean' | 'rose' | 'light' | 'gold' | 'system';
