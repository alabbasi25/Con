...
interface PlanetContextType extends KokabState {
  ...
  updateTaskColumn: (taskId: string, column: 'needed' | 'in-cart' | 'purchased') => void;
...
export const PlanetProvider: React.FC<{ children: React.ReactNode; userId: UserID }> = ({ children, userId }) => {
  ...
  const [theme, setThemeState] = useState<Theme>((localStorage.getItem('kokab-theme') as Theme) || 'midnight');
  const [appSize, setAppSizeState] = useState<AppSize>((localStorage.getItem('kokab-appsize') as AppSize) || 'large');
  ...
  const setAppSize = (newSize: AppSize) => {
    setAppSizeState(newSize);
    localStorage.setItem('kokab-appsize', newSize);
    document.documentElement.setAttribute('data-appsize', newSize);
  };
  ...
  useEffect(() => {
    document.documentElement.setAttribute('data-appsize', appSize);
  }, [appSize]);
  ...
  const addTask = (task: Partial<Task>) => {
    const newTask: Task = {
      ...
      kanbanColumn: task.kanbanColumn || 'needed'
    };
    ...
    if (newTask.assignedTo !== currentUser) {
      sendPartnerNotification({
        title: 'مهمة جديدة لك! 📋',
        content: `${profiles[currentUser].name} قام بتكليفك بمهمة: ${newTask.title}`,
        type: 'tasks'
      });
    }
  ...
  const completeTask = (taskId: string) => {
    setTasks(prev => prev.map(t => t.id === taskId ? { ...t, status: 'completed', kanbanColumn: 'purchased' } : t));
  };

  const updateTaskColumn = (taskId: string, column: 'needed' | 'in-cart' | 'purchased') => {
    setTasks(prev => prev.map(t => t.id === taskId ? { ...t, kanbanColumn: column, status: column === 'purchased' ? 'completed' : 'pending' } : t));
  };
  ...
  const deleteTask = (taskId: string) => {
    ...
    // Security check: Only creator or assignee can delete
    if (task.creatorId !== currentUser && task.assignedTo !== currentUser) {
      addNotification({
        title: 'عذراً! 🚫',
        content: 'لا يمكنك حذف مهمة لست من أنشأها أو كُلف بها.',
        type: 'urgent'
      });
      return;
    }
  ...
  const delegateTask = (taskId: string, to: UserID, reason: string) => {
    ...
    if (to !== currentUser) {
      sendPartnerNotification({
        title: 'تم تفويض مهمة لك! 🤝',
        content: `${profiles[currentUser].name} فوض لك مهمة "${task?.title}" لسبب: ${reason}`,
        type: 'tasks'
      });
    }
  };
  ...
  const addTransaction = (t: Partial<Transaction>) => {
    ...
    // Check budget limit
    const totalSpent = transactions.reduce((acc, tr) => acc + tr.amount, 0) + newT.amount;
    if (totalSpent >= budget.total * 0.9) {
      addNotification({
        title: 'تنبيه الميزانية! ⚠️',
        content: 'لقد استهلكتم 90% من الميزانية المحددة لهذا الشهر.',
        type: 'urgent'
      });
    }
    ...
    if (newT.amount > 500) {
      sendPartnerNotification({
        title: 'مصروف كبير! 💸',
        content: `${profiles[currentUser].name} أضاف مصروفاً بقيمة ${newT.amount} ر.س: ${newT.description}`,
        type: 'financial'
      });
    }
  ...
  const addNotification = (n: Partial<Notification>) => {
    ...
    const targetUserId = n.userId || currentUser;
    const settings = profiles[targetUserId].notificationSettings;
    ...
    // Support new granular settings in AI Oracle
    if (n.category === 'budget' && !settings.smartBudget) shouldAdd = false;
    if (n.category === 'affection' && !settings.affectionNudges) shouldAdd = false;

    if (!shouldAdd && type !== 'urgent') return;
    if (settings.muteMode === 'all') return;
    ...
    // Sound alert logic
    if (settings.soundEnabled && settings.muteMode === 'none') {
      console.log('Planetary Alert: playing notification sound...');
    }
  };
  ...
  const clearNotifications = () => {
    setNotifications(prev => prev.filter(n => n.userId !== currentUser));
  };

  const sendPartnerNotification = (n: Partial<Notification>) => {
    const partnerId = currentUser === 'F' ? 'B' : 'F';
    
    // For local simulation, also add it directly so it shows when switching
    addNotification({ ...n, userId: partnerId });
  ...
  const deleteLiability = (id: string) => {
    ...
    if (item && item.userId !== currentUser) {
      addNotification({ title: 'تنبيه أمني ⚠️', content: 'لا يمكنك حذف التزام مالي خاص بالشريك.', type: 'urgent' });
      return;
    }
  ...
  const deleteAssetGoal = (id: string) => {
    const goal = assets.find(a => a.id === id);
    if (goal && goal.userId !== currentUser) {
      addNotification({ title: 'تنبيه أمني ⚠️', content: 'لا يمكنك حذف هدف استثماري خاص بالشريك.', type: 'urgent' });
      return;
    }
  ...
  const deleteCalendarEvent = (id: string) => {
    const ev = calendar.find(e => e.id === id);
    if (ev && ev.creatorId !== currentUser) {
      addNotification({ title: 'تنبيه أمني ⚠️', content: 'لا يمكنك حذف حدث تقويم أنشأه الشريك.', type: 'urgent' });
      return;
    }
  ...
  const connectFitness = async () => {
    try {
      ...
      if (!popup) alert('يرجى السماح بالنوافذ المنبثقة لربط Google Fit.');
    } catch (error) {
      console.error('Failed to connect fitness:', error);
      // Fallback: Just trigger a sync to simulate successful auth since there's no backend
      syncFitness();
      addNotification({
        title: 'تم الربط محلياً',
        content: 'تعذر الوصول للخادم، تم الاتصال بالبيانات المحلية بنجاح.',
        type: 'routine'
      });
    }
  };
  ...
  const syncFitness = async () => {
    try {
      ...
      // Fallback to mock local data so the UI remains interactive
      setVitals(prev => ({
        ...prev,
        [currentUser]: {
          ...prev[currentUser],
          steps: prev[currentUser].steps + Math.floor(Math.random() * 200),
          calories: prev[currentUser].calories + Math.floor(Math.random() * 50),
          lastSync: Date.now()
        }
      }));
    }
  ...
  const populateTestData = () => {
    ...
    const now = Date.now();
    setTasks([
      { id: 't1', creatorId: 'B', title: 'شراء مستلزمات العشاء', assignedTo: 'F', status: 'pending', priority: 'medium', category: 'home', estimatedMinutes: 45, createdAt: now - 3600000, dueDate: now + 7200000 },
      { id: 't2', creatorId: 'F', title: 'تنظيف الحديقة', assignedTo: 'B', status: 'pending', priority: 'low', category: 'home', estimatedMinutes: 60, createdAt: now - 7200000, dueDate: now - 3600000 }, // Overdue
      { id: 't3', creatorId: 'F', title: 'دفع فاتورة الكهرباء', assignedTo: 'F', status: 'completed', priority: 'urgent', category: 'work', estimatedMinutes: 10, createdAt: now - 86400000 },
      { id: 't4', creatorId: 'B', title: 'إصلاح صنبور المياه', assignedTo: 'F', status: 'pending', priority: 'high', category: 'home', estimatedMinutes: 30, createdAt: now - 1800000 },
    ]);
    ...
    setLiabilities([
      { id: 'l1', userId: 'F', name: 'قسط السيارة', totalAmount: 50000, remainingAmount: 12000, monthlyInstallment: 1500, dueDate: now + 15 * 86400000 },
      { id: 'l2', userId: 'B', name: 'تمويل شخصي', totalAmount: 20000, remainingAmount: 5000, monthlyInstallment: 800, dueDate: now + 5 * 86400000 }
    ]);
  ...
  const resetApp = () => {
    ...
    setNotifications([{ id: 'n1', userId: currentUser, title: 'تم تصفير النظام', content: 'تم مسح كافة البيانات بنجاح.', timestamp: Date.now(), read: false, type: 'routine' }]);
  };
  ...
  const deletePrivateNote = (id: string) => {
    const note = privateNotes.find(n => n.id === id);
    if (note && note.userId !== currentUser) {
      addNotification({ title: 'تنبيه أمني ⚠️', content: 'لا يمكنك حذف ملاحظات الشريك الخاصة.', type: 'urgent' });
      return;
    }
  ...
  const deleteJournalEntry = (id: string) => {
    ...
    if (entry && entry.authorId !== currentUser) {
      addNotification({ title: 'تنبيه أمني ⚠️', content: 'لا يمكنك حذف مذكرات الشريك.', type: 'urgent' });
      return;
    }
  ...
  const submitLoveLanguageResult = (scores: Record<LoveLanguage, number>) => {
    ...
    sendPartnerNotification({
      title: 'اكتشاف جديد! ❤️',
      content: `${profiles[currentUser].name} أكمل اختبار لغات الحب. لغته الأساسية هي: ${primary === 'words' ? 'كلمات التشجيع' : primary === 'acts' ? 'الأعمال الخدمية' : primary === 'gifts' ? 'الهدايا' : primary === 'time' ? 'الوقت النوعي' : 'الاتصال الجسدي'}`,
      type: 'social'
    });
  };
  ...
  const revealConflictMessages = () => {
    ...
    addNotification({
      title: 'لحظة صراحة ✨',
      content: 'تم الكشف عن مشاعر الطرفين في غرفة احتواء الخلاف.',
      type: 'social'
    });
    
    sendPartnerNotification({
      title: 'تم الكشف عن المشاعر 🕊️',
      content: 'يمكنك الآن رؤية ما كتبه شريكك في غرفة احتواء الخلاف.',
      type: 'social'
    });
  };

  const sendChatMessage = (content: string, type: ChatMessage['type'] = 'text') => {
    ...
    sendPartnerNotification({
      title: 'رسالة جديدة ✨',
      content: `${currentUser === 'F' ? 'فهد' : 'بشرى'} أرسل لك رسالة في الدردشة.`,
      type: 'social'
    });
  };
  ...
  return (
    <PlanetContext.Provider value={{
      ...
      addPrivateNote, deletePrivateNote, addGratitude, likeGratitudePost, commentGratitudePost,
      addMoodLog, addJournalEntry, updateBudget, submitLoveLanguageResult,
      sendConflictMessage, revealConflictMessages, sendChatMessage, updateNotificationSettings, updateTaskSettings,
      theme, setTheme, appSize, setAppSize, chatMessages
    }}>
      {children}
